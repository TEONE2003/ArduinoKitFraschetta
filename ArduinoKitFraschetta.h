...
class SERVOMOTORE:Servo,MOTORE_SWIPING{
 protected:
  uint8_t PIN;
  uint16_t POSIZIONE_CORRENTE()override{return EEPROM.read(PIN);}
  ...
  void SWIPE_AVANTI()override{IMPOSTA_POSIZIONE(GRADI_AVANTI, PERIODO_SPOSTAMENTO_IN_MILLISECONDI);}
  void SWIPE_INDIETRO()override{IMPOSTA_POSIZIONE(GRADI_INDIETRO, PERIODO_SPOSTAMENTO_IN_MILLISECONDI);}
public:
 ...
 void SWIPING(uint8_t GRADI_AVANTI, uint8_t GRADI_INDIETRO,uint16_t TEMPO_DA_FERMO, UNITA_DI_TEMPO UNITA){
...
protected:
 uint16_t PASSI_MASSIMI,INDIRIZZO_POSIZIONE_EEPROM;
public:
 uint16_t PERIODO_SPOSTAMENTO_IN_MILLISECONDI;
 ...
 void PASSI_A_DESTRA(uint16_t PASSI){
  for(uint16_t P=POSIZIONE_CORRENTE(); P<=PASSI; P++){PASSO_A_DESTRA(); delay(PERIODO_SPOSTAMENTO_IN_MILLISECONDI);}
 }
 void PASSI_A_SINISTRA(uint16_t PASSI){
  for(uint16_t P=POSIZIONE_CORRENTE(); P>=PASSI; P--){PASSO_A_SINISTRA(); delay(PERIODO_SPOSTAMENTO_IN_MILLISECONDI);}
 }
 void PASSI(uint16_t PASSI){PASSI_A_DESTRA(PASSI); PASSO_A_SINISTRA(PASSI);}
 ...
 void SWIPING(uint16_t PASSI_AVANTI,uint16_t PASSI_INDIETRO,uint16_t TEMPO_DA_FERMO, UNITA_DI_TEMPO UNITA){
  GRADI_AVANTI = PASSI_AVANTI;
  GRADI_INDIETRO = PASSI_INDIETRO;
  MOTORE_SWIPING::SWIPING(TEMPO_DA_FERMO,UNITA);
 }
};
...
public:
  ...
  MOTORE_STEPPER_BIPOLARE(uint16_t PASSI_MASSIMI,uint8_t PIN_BOBINA1,uint8_t PIN_BOBINA2,uint8_t PIN_BOBINA3,uint8_t PIN_BOBINA4,uint16_t VELOCITA):
...
/*class MOTORE_STEPPER_UNIPOLARE:Stepper,public MOTORE_STEPPER_BASE{
 public:
  ...
  MOTORE_STEPPER_UNIPOLARE(uint16_t PASSI_MASSIMI,uint8_t PIN_BOBINA1,uint8_t PIN_BOBINA2,uint8_t PIN_BOBINA3,uint8_t PIN_BOBINA4,uint16_t VELOCITA):
   MOTORE_STEPPER_BASE(PASSI_MASSIMI,VELOCITA,PIN_BOBINA1),
   Stepper(PASSI_MASSIMI,PIN_BOBINA1,PIN_BOBINA2,PIN_BOBINA3,PIN_BOBINA4){}
};*/
...
class STEPPER_INGRESSO_BIPOLARE:public MOTORE_STEPPER_BIPOLARE,public STEPPER_INGRESSO_BASE{
 public:
  void CONTROLLO_STATO_MOVIMENTO(){STEPPER_INGRESSO_BASE::CONTROLLO_STATO_MOVIMENTO();}
   uint16_t POSIZIONE_CORRENTE() override {return MOTORE_STEPPER_BIPOLARE::POSIZIONE_CORRENTE();}
    STEPPER_INGRESSO_BIPOLARE(uint16_t PASSI_MASSIMI,uint8_t PIN_BOBINA1,uint8_t PIN_BOBINA2,uint8_t PIN_BOBINA3,uint8_t PIN_BOBINA4,
   uint16_t PASSI_APERTURA,uint16_t PASSI_CHIUSURA,uint8_t VELOCITA):
    MOTORE_STEPPER_BIPOLARE(PASSI_MASSIMI,VELOCITA,PIN_BOBINA1,PIN_BOBINA2,PIN_BOBINA3,PIN_BOBINA4),
   STEPPER_INGRESSO_BASE(PASSI_MASSIMI,PASSI_APERTURA,PASSI_CHIUSURA,VELOCITA,PIN_BOBINA1,PIN_BOBINA2){}
};

/*class STEPPER_INGRESSO_UINIPOLARE:public MOTORE_STEPPER_UNIPOLARE,public STEPPER_INGRESSO_BASE{
 public:
  void CONTROLLO_STATO_MOVIMENTO(){STEPPER_INGRESSO_BASE::CONTROLLO_STATO_MOVIMENTO();}
   uint16_t POSIZIONE_CORRENTE() override {return MOTORE_STEPPER_UNIPOLARE::POSIZIONE_CORRENTE();}
    STEPPER_INGRESSO_UINIPOLARE(uint16_t PASSI_MASSIMI,uint8_t PIN_BOBINA1,uint8_t PIN_BOBINA2,uint8_t PIN_BOBINA3,uint8_t PIN_BOBINA4,
   uint16_t PASSI_APERTURA,uint16_t PASSI_CHIUSURA,uint8_t VELOCITA):
    MOTORE_STEPPER_UNIPOLARE(PASSI_MASSIMI,VELOCITA,PIN_BOBINA1,PIN_BOBINA2,PIN_BOBINA3,PIN_BOBINA4),
   STEPPER_INGRESSO_BASE(PASSI_MASSIMI,PASSI_APERTURA,PASSI_CHIUSURA,VELOCITA,PIN_BOBINA1,PIN_BOBINA2){}
};*/
