...
void AT_RESETTA_IMPOSTAZIONI(){print("AT+RESET");}
void AT_IMPOSTA_NOME(String NOME){print("AT+NAME"+NOME);}
String AT_NOME(){
 print("AT+NAME?");
...
void AT_IMPOSTA_RUOLO(RUOLO R){
 if(R==MASTER){print("AT+ROLE1");}
 else{print("AT+ROLE0");}
}
IMPOSTA_BAUD(uint8_t BAUD){print("AT+BAUD"+String(BAUD));}
};
...
void FUNZIONE_TERMOSTATO(uint8_t TEMPERATURA_ATTUALE){
 if(TICK()){
    switch(TIPO_T){
     case RISCALDAMENTO:
        if(TEMPERATURA_ATTUALE<SOGLIA_TEMPERATURA){ACCENDI();} else{SPEGNI();}
        break;
     case RAFFREDDAMENTO:
        if(TEMPERATURA_ATTUALE>SOGLIA_TEMPERATURA){ACCENDI();} else {SPEGNI();}
        break;
...
void FUNZIONE_CREPUSCOLARE(){
  if(CONTROLLO()){ACCENDI();} else{SPEGNI();}
}
};
