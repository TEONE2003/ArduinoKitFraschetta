...
void CONVERTI_IN(UNITA_DI_TEMPO UNITA){this->VALORE=CONVERTITO_IN(UNITA).VALORE; this->UNITA=UNITA; }
 ...
 TEMPO operator-(const TEMPO &B){B.CONVERTI_IN(UNITA); TEMPO T(VALORE-B.VALORE,UNITA); return T;}
 TEMPO operator*(const TEMPO &B){B.CONVERTI_IN(UNITA); TEMPO T(VALORE*B.VALORE,UNITA); return T;}
 uint64_t operator/(const TEMPO &B){B.CONVERTI_IN(UNITA); return VALORE/B.VALORE;}
 TEMPO operator%(const TEMPO &B){B.CONVERTI_IN(UNITA); TEMPO T(VALORE%B.VALORE,UNITA); return T;}
...
public:
 CicloVirtualeF(uint64_t PERIODO=0,UNITA_DI_TEMPO UNITA=MILLISECONDI):CronometroVirtualeF(UNITA){}
