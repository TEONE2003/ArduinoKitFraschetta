   ...
   if(!C.INIZIALIZZATO()){C = CICLO_TIMER(TEMPO_DA_FERMO,UNITA);}
    if(C.TICK()){
     if(POSIZIONE_AVANTI>POSIZIONE_INDIETRO){
      if(POSIZIONE_CORRENTE()<POSIZIONE_AVANTI){SWIPE_AVANTI();}
      else if(POSIZIONE_CORRENTE()>POSIZIONE_INDIETRO){SWIPE_INDIETRO();}
     }
     else{
      if(POSIZIONE_CORRENTE()>POSIZIONE_AVANTI){SWIPE_AVANTI();}
      else if(POSIZIONE_CORRENTE()<POSIZIONE_INDIETRO){SWIPE_INDIETRO();}
...
public:
 ...
 void SWIPING(uint16_t PASSI_AVANTI,uint16_t PASSI_INDIETRO,uint16_t TEMPO_DA_FERMO, UNITA_DI_TEMPO UNITA){
  POSIZIONE_AVANTI = PASSI_AVANTI;
  POSIZIONE_INDIETRO = PASSI_INDIETRO;
  MOTORE_SWIPING::SWIPING(TEMPO_DA_FERMO,UNITA);
 }
};
