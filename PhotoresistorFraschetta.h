...
protected:
 ...
 bool LastCheckExecuted,SerialInizialized;
public:
 PhotoresistorF(uint8_t Pin,uint16_t TurnOnThreshold,uint16_t TurnOffThreshold,uint64_t Delay,UnitOfTime Unit):InputF(Pin),VirtualCycleF(Delay,Unit){
  ...
  LastCheckExecuted=0; SerialInizialized=0;
 }
 uint16_t ReadPhotoresistor(){return AnalogRead();}
 void Calibrate(){if(!SerialInizialized){Serial.end(); Serial.begin(9600); SerialInizialized=1;} Serial.print("Photoresistor: "+String(ReadPhotoresistor())+"\n");}
 boolean Check(){
 if(nTick()){
